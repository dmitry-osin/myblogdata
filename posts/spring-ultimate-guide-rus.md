- [Модуль 0. Java / JVM / Concurrency / Performance](#модуль-0-java--jvm--concurrency--performance)
  - [Часть 0.1: JVM Internals (Практический минимум для Highload)](#часть-01-jvm-internals-практический-минимум-для-highload)
    - [1. Память: Heap, Metaspace, Stack и скрытые детали](#1-память-heap-metaspace-stack-и-скрытые-детали)
      - [Heap (Куча)](#heap-куча)
      - [Metaspace (Off-heap)](#metaspace-off-heap)
      - [Compressed Oops (Ordinary Object Pointers)](#compressed-oops-ordinary-object-pointers)
    - [2. GC в проде: G1 vs ZGC](#2-gc-в-проде-g1-vs-zgc)
      - [G1 GC (Garbage First) — Дефолт с Java 9](#g1-gc-garbage-first--дефолт-с-java-9)
      - [ZGC (The Z Garbage Collector) — Production Ready в Java 21](#zgc-the-z-garbage-collector--production-ready-в-java-21)
    - [3. JIT / HotSpot: Магия оптимизации](#3-jit--hotspot-магия-оптимизации)
      - [Inlining (Инлайнинг)](#inlining-инлайнинг)
      - [Escape Analysis \& Scalar Replacement](#escape-analysis--scalar-replacement)
      - [Deoptimization](#deoptimization)
    - [4. Classloading: Типовые боли](#4-classloading-типовые-боли)
    - [Практическое задание (Mental Check)](#практическое-задание-mental-check)
- [Модуль 0.2 Concurrency углублённо](#модуль-02-concurrency-углублённо)
    - [1. Java Memory Model (JMM): Контракт с реальностью](#1-java-memory-model-jmm-контракт-с-реальностью)
      - [Три кита безопасности потоков:](#три-кита-безопасности-потоков)
      - [Happens-Before](#happens-before)
      - [Volatile (Практика)](#volatile-практика)
    - [2. CompletableFuture: Асинхронный "клей"](#2-completablefuture-асинхронный-клей)
    - [3. ForkJoinPool: Work-Stealing](#3-forkjoinpool-work-stealing)
    - [4. Virtual Threads (Project Loom, Java 21): Геймчейнджер](#4-virtual-threads-project-loom-java-21-геймчейнджер)
      - [Концепция](#концепция)
      - [M:N Model](#mn-model)
      - [Подводные камни (Pinning)](#подводные-камни-pinning)
    - [5. Structured Concurrency (Java 21 Preview)](#5-structured-concurrency-java-21-preview)
    - [Практический вывод для собеседования](#практический-вывод-для-собеседования)
- [Модуль 0.3. Диагностика и профилирование](#модуль-03-диагностика-и-профилирование)
    - [1. JFR (Java Flight Recorder) и JMC (Java Mission Control)](#1-jfr-java-flight-recorder-и-jmc-java-mission-control)
    - [2. Thread Dump (Снимок потоков)](#2-thread-dump-снимок-потоков)
    - [3. Heap Dump (Снимок памяти)](#3-heap-dump-снимок-памяти)
    - [4. async-profiler (Тяжелая артиллерия)](#4-async-profiler-тяжелая-артиллерия)
    - [5. Нагрузочное тестирование (Load Testing)](#5-нагрузочное-тестирование-load-testing)
    - [Практическое резюме](#практическое-резюме)
  - [Часть 1.1: IoC Container \& Bean Lifecycle (Механика "под капотом")](#часть-11-ioc-container--bean-lifecycle-механика-под-капотом)
    - [ApplicationContext vs BeanFactory](#applicationcontext-vs-beanfactory)
    - [Bean Scopes: Скрытые нюансы](#bean-scopes-скрытые-нюансы)
    - [Жизненный цикл бина (The Bean Lifecycle)](#жизненный-цикл-бина-the-bean-lifecycle)
  - [Часть 1.2: Конфигурация и Внедрение (DI)](#часть-12-конфигурация-и-внедрение-di)
    - [Java-based Configuration: `@Configuration` vs `@Component`](#java-based-configuration-configuration-vs-component)
    - [Типы внедрения зависимостей (Best Practices)](#типы-внедрения-зависимостей-best-practices)
      - [1. Field Injection (Избегать!)](#1-field-injection-избегать)
      - [2. Setter Injection (Опционально)](#2-setter-injection-опционально)
      - [3. Constructor Injection (Рекомендовано)](#3-constructor-injection-рекомендовано)
    - [Циклические зависимости (Circular Dependencies)](#циклические-зависимости-circular-dependencies)
    - [Разбор кода: BeanPostProcessor (BPP)](#разбор-кода-beanpostprocessor-bpp)
    - [Практическое задание (Mental Check)](#практическое-задание-mental-check-1)
- [Модуль 1.3. SpEL и Внешняя Конфигурация](#модуль-13-spel-и-внешняя-конфигурация)
    - [1. `@Value` vs `@ConfigurationProperties`](#1-value-vs-configurationproperties)
      - [`@Value("${app.timeout}")` — "Быстро, но грязно"](#valueapptimeout--быстро-но-грязно)
      - [`@ConfigurationProperties` — "Стандарт Enterprise"](#configurationproperties--стандарт-enterprise)
    - [2. SpEL (Spring Expression Language) — Мощно, но опасно](#2-spel-spring-expression-language--мощно-но-опасно)
- [Модуль 2. Spring Boot 3.x Internals (Magic Revealed)](#модуль-2-spring-boot-3x-internals-magic-revealed)
    - [2.1. AutoConfiguration: Как это работает в Boot 3.x](#21-autoconfiguration-как-это-работает-в-boot-3x)
    - [2.2. Conditionals (`@Conditional...`) — Фейсконтроль бинов](#22-conditionals-conditional--фейсконтроль-бинов)
    - [2.3. Debugging AutoConfig](#23-debugging-autoconfig)
    - [Практическое задание: Пишем свой Starter](#практическое-задание-пишем-свой-starter)
    - [Резюме по Модулю 2](#резюме-по-модулю-2)
  - [Механика Spring AOP: Проксирование](#механика-spring-aop-проксирование)
    - [1. JDK Dynamic Proxy vs. CGLIB](#1-jdk-dynamic-proxy-vs-cglib)
  - [Практика: Пишем полезный аспект для банка](#практика-пишем-полезный-аспект-для-банка)
    - [Шаг 1: Создаем аннотацию](#шаг-1-создаем-аннотацию)
    - [Шаг 2: Реализуем Аспект](#шаг-2-реализуем-аспект)
  - [Важные нюансы для Production](#важные-нюансы-для-production)
    - [1. Порядок Аспектов (`@Order`)](#1-порядок-аспектов-order)
    - [2. Безопасность данных (PII)](#2-безопасность-данных-pii)
    - [3. Производительность (Overhead)](#3-производительность-overhead)
  - [Разбор "Self-Invocation" через призму AOP](#разбор-self-invocation-через-призму-aop)
  - [Мини-чеклист по AOP](#мини-чеклист-по-aop)
  - [1. Entity Lifecycle и Persistence Context (L1 Cache)](#1-entity-lifecycle-и-persistence-context-l1-cache)
    - [Четыре состояния сущности:](#четыре-состояния-сущности)
    - [Dirty Checking (Грязная проверка)](#dirty-checking-грязная-проверка)
  - [2. Проблема N+1 (The Silent Killer)](#2-проблема-n1-the-silent-killer)
    - [Решения (Как лечить):](#решения-как-лечить)
      - [А. `JOIN FETCH` (JPQL)](#а-join-fetch-jpql)
      - [Б. `@EntityGraph` (Spring Data JPA)](#б-entitygraph-spring-data-jpa)
  - [3. Проблема генерации ID (Identity vs Sequence)](#3-проблема-генерации-id-identity-vs-sequence)
    - [`GenerationType.IDENTITY` (Auto-increment в MySQL/PG)](#generationtypeidentity-auto-increment-в-mysqlpg)
    - [`GenerationType.SEQUENCE` (PostgreSQL / Oracle)](#generationtypesequence-postgresql--oracle)
  - [4. LazyInitializationException](#4-lazyinitializationexception)
  - [5. Locking (Конкурентный доступ)](#5-locking-конкурентный-доступ)
    ...
    - [Практическое задание (Mental Check)](#практическое-задание-mental-check-2)
- [Модуль 4.2. Spring Data JPA (Best Practices)](#модуль-42-spring-data-jpa-best-practices)
    - [1. Методы-монстры vs `@Query`](#1-методы-монстры-vs-query)
    - [2. Projections (DTO Projection) — Экономим память](#2-projections-dto-projection--экономим-память)
    - [3. Пагинация: `Page` vs `Slice`](#3-пагинация-page-vs-slice)
- [Модуль 4.3. Управление транзакциями (`@Transactional`)](#модуль-43-управление-транзакциями-transactional)
    - [1. Propagation (Распространение)](#1-propagation-распространение)
    - [2. Isolation (Изоляция)](#2-isolation-изоляция)
    - [3. Главный анти-паттерн: Long Transaction (Долгая транзакция)](#3-главный-анти-паттерн-long-transaction-долгая-транзакция)
- [Модуль 4.4. SQL для разработчика (PostgreSQL Focus)](#модуль-44-sql-для-разработчика-postgresql-focus)
    - [1. Индексы: B-Tree vs GIN](#1-индексы-b-tree-vs-gin)
    - [2. `EXPLAIN (ANALYZE)`](#2-explain-analyze)
    - [3. HikariCP Tuning](#3-hikaricp-tuning)
    - [Практическое резюме по Data Layer](#практическое-резюме-по-data-layer)
  - [6.1. Асинхронность внутри JVM (`@Async`)](#61-асинхронность-внутри-jvm-async)
    - [Главная ловушка: Default Executor](#главная-ловушка-default-executor)
    - [Правильная настройка (Custom Executor)](#правильная-настройка-custom-executor)
    - [Context Propagation (MDC)](#context-propagation-mdc)
  - [6.2. Apache Kafka (Хребет банковской архитектуры)](#62-apache-kafka-хребет-банковской-архитектуры)
    - [Consumer Groups \& Rebalancing](#consumer-groups--rebalancing)
    - [Delivery Semantics (Семантика доставки)](#delivery-semantics-семантика-доставки)
    - [Poison Pill (Отравленное сообщение)](#poison-pill-отравленное-сообщение)
  - [6.3. HTTP Clients (Feign \& RestClient)](#63-http-clients-feign--restclient)
    - [Feign Client (Declarative)](#feign-client-declarative)
    - [Resiliency (Устойчивость)](#resiliency-устойчивость)
  - [6.4. Планировщики (Schedulers)](#64-планировщики-schedulers)
    - [`@Scheduled` в кластере Kubernetes](#scheduled-в-кластере-kubernetes)
    - [Решение: Distributed Lock (ShedLock / Quartz)](#решение-distributed-lock-shedlock--quartz)
  - [Практическое задание (Mental Check)](#практическое-задание-mental-check-3)
  - [7.3. Transactional Outbox (Главный паттерн интеграции)](#73-transactional-outbox-главный-паттерн-интеграции)
    - [Решение: Outbox Pattern](#решение-outbox-pattern)
  - [7.4. Saga Pattern (Длинные транзакции)](#74-saga-pattern-длинные-транзакции)
    - [Choreography (Хореография) — Event Driven](#choreography-хореография--event-driven)
    - [Orchestration (Оркестрация) — Предпочтительно для банков](#orchestration-оркестрация--предпочтительно-для-банков)
  - [7.1. CQRS (Command Query Responsibility Segregation)](#71-cqrs-command-query-responsibility-segregation)
  - [7.6. Resiliency (Устойчивость)](#76-resiliency-устойчивость)
    - [Circuit Breaker (Предохранитель)](#circuit-breaker-предохранитель)
  - [7.7. Cache (Redis Patterns)](#77-cache-redis-patterns)
    - [Cache Stampede (Dogpile Effect)](#cache-stampede-dogpile-effect)
  - [Практическое задание (Mental Check)](#практическое-задание-mental-check-4)
  - [8.1. Spring Security Internals: The Filter Chain](#81-spring-security-internals-the-filter-chain)
    - [Как это работает:](#как-это-работает)
    - [Конфигурация в Spring Boot 3 (Lambda DSL)](#конфигурация-в-spring-boot-3-lambda-dsl)
  - [8.2. OAuth2 / OIDC: Современный стандарт](#82-oauth2--oidc-современный-стандарт)
    - [Роли:](#роли)
    - [JWT (JSON Web Token) — Анатомия](#jwt-json-web-token--анатомия)
    - [Валидация токена (Stateless vs Stateful)](#валидация-токена-stateless-vs-stateful)
  - [8.3. Method Security (`@PreAuthorize`)](#83-method-security-preauthorize)
    - [ABAC (Attribute-Based Access Control)](#abac-attribute-based-access-control)
  - [8.4. AppSec: Secure by Default](#84-appsec-secure-by-default)
    - [CORS (Cross-Origin Resource Sharing)](#cors-cross-origin-resource-sharing)
    - [CSRF (Cross-Site Request Forgery)](#csrf-cross-site-request-forgery)
    - [Secrets Management](#secrets-management)
  - [Практическое задание (Mental Check)](#практическое-задание-mental-check-5)
  - [9.1. Spring Boot Actuator: Пульс приложения](#91-spring-boot-actuator-пульс-приложения)
    - [Liveness vs Readiness Probes (Для Kubernetes)](#liveness-vs-readiness-probes-для-kubernetes)
    - [Security Warning](#security-warning)
  - [9.2. Structured Logging \& MDC](#92-structured-logging--mdc)
    - [JSON Logs](#json-logs)
    - [MDC (Mapped Diagnostic Context)](#mdc-mapped-diagnostic-context)
  - [9.3. Metrics (Micrometer)](#93-metrics-micrometer)
    - [Типы метрик](#типы-метрик)
    - [Dimensional Metrics (Теги)](#dimensional-metrics-теги)
    - [The Golden Signals (Что мониторить в первую очередь)](#the-golden-signals-что-мониторить-в-первую-очередь)
  - [9.4. Distributed Tracing (OpenTelemetry)](#94-distributed-tracing-opentelemetry)
  - [9.5. SLO / SLI (Культура надежности)](#95-slo--sli-культура-надежности)
  - [Практическое задание (Mental Check)](#практическое-задание-mental-check-6)
  - [10.1. Unit Testing: Честность и скорость](#101-unit-testing-честность-и-скорость)
    - [Mockito: Strictness \& BDD](#mockito-strictness--bdd)
    - [Mutation Testing (PITest)](#mutation-testing-pitest)
  - [10.2. Spring Test Slices (Срезы)](#102-spring-test-slices-срезы)
    - [`@WebMvcTest` (Только контроллеры)](#webmvctest-только-контроллеры)
    - [`@DataJpaTest` (Только БД)](#datajpatest-только-бд)
  - [10.3. Integration Testing \& Testcontainers (Золотой стандарт)](#103-integration-testing--testcontainers-золотой-стандарт)
    - [Singleton Container Pattern (Оптимизация)](#singleton-container-pattern-оптимизация)
  - [10.4. ArchUnit: Тестируем Архитектуру](#104-archunit-тестируем-архитектуру)
  - [10.5. Dirty Context (Боль Спринга)](#105-dirty-context-боль-спринга)
  - [Практическое задание (Mental Check)](#практическое-задание-mental-check-7)
  - [11.1. Build Systems (Maven/Gradle): Гигиена зависимостей](#111-build-systems-mavengradle-гигиена-зависимостей)
    - [BOM (Bill of Materials)](#bom-bill-of-materials)
    - [Maven Enforcer Plugin](#maven-enforcer-plugin)
    - [Reproducible Builds (Воспроизводимые сборки)](#reproducible-builds-воспроизводимые-сборки)
  - [11.2. Docker: The Java Way](#112-docker-the-java-way)
    - [Multi-stage builds](#multi-stage-builds)
    - [Layered JARs (Слои)](#layered-jars-слои)
    - [Distroless Images](#distroless-images)
  - [11.3. Supply Chain Security (Безопасность поставки)](#113-supply-chain-security-безопасность-поставки)
    - [SBOM (Software Bill of Materials)](#sbom-software-bill-of-materials)
    - [Container Signing (Cosign / Sigstore)](#container-signing-cosign--sigstore)
  - [11.4. Kubernetes для Разработчика](#114-kubernetes-для-разработчика)
    - [Resources: Requests vs Limits](#resources-requests-vs-limits)
    - [Graceful Shutdown (Мягкая остановка)](#graceful-shutdown-мягкая-остановка)
  - [11.5. CI/CD Pipeline Logic](#115-cicd-pipeline-logic)
  - [Практическое задание (Mental Check)](#практическое-задание-mental-check-8)
  - [12.1. DDD (Domain-Driven Design): Основы](#121-ddd-domain-driven-design-основы)
    - [Ubiquitous Language (Единый язык)](#ubiquitous-language-единый-язык)
    - [Bounded Contexts (Ограниченные контексты)](#bounded-contexts-ограниченные-контексты)
    - [Aggregate (Агрегат) — Граница транзакции](#aggregate-агрегат--граница-транзакции)
  - [12.2. Hexagonal Architecture (Ports \& Adapters)](#122-hexagonal-architecture-ports--adapters)
    - [Принцип:](#принцип)
  - [12.3. Spring Modulith: Модульный Монолит](#123-spring-modulith-модульный-монолит)
    - [Структура пакетов:](#структура-пакетов)
    - [Event-Driven interaction](#event-driven-interaction)
  - [12.4. Documentation as Code (ADR)](#124-documentation-as-code-adr)
  - [12.5. Главные Анти-паттерны](#125-главные-анти-паттерны)
  - [13.1. Spring Cloud Gateway (API Gateway)](#131-spring-cloud-gateway-api-gateway)
    - [Ключевые фичи для банка:](#ключевые-фичи-для-банка)
  - [13.2. Spring Cloud Config (Externalized Configuration)](#132-spring-cloud-config-externalized-configuration)
    - [@RefreshScope](#refreshscope)
  - [13.3. Spring Cloud Kubernetes](#133-spring-cloud-kubernetes)
  - [13.4. Spring Cloud Stream (Event Driven Abstraction)](#134-spring-cloud-stream-event-driven-abstraction)
    - [Зачем это нужно?](#зачем-это-нужно)
  - [13.5. Spring Cloud LoadBalancer (Client-Side LB)](#135-spring-cloud-loadbalancer-client-side-lb)
    - [Legacy-чеклист (Что НЕ надо тащить в 2026)](#legacy-чеклист-что-не-надо-тащить-в-2026)
    - [Практическое дополнение к Roadmap:](#практическое-дополнение-к-roadmap)
- [Дополнительно: Maven Deep Dive (Build Engineering)](#дополнительно-maven-deep-dive-build-engineering)
  - [1. Философия: Lifecycle, Phases и Goals](#1-философия-lifecycle-phases-и-goals)
    - [Три основных цикла:](#три-основных-цикла)
    - [Прод-нюанс: `package` vs `install` vs `deploy`](#прод-нюанс-package-vs-install-vs-deploy)
  - [2. Управление зависимостями (Dependency Management)](#2-управление-зависимостями-dependency-management)
    - [Transitive Dependencies (Транзитивность)](#transitive-dependencies-транзитивность)
    - [Scopes (Области видимости)](#scopes-области-видимости)
    - [BOM (Bill of Materials)](#bom-bill-of-materials-1)
  - [3. Топ Плагинов для Enterprise (Must-Have)](#3-топ-плагинов-для-enterprise-must-have)
    - [Базовые плагины (Core)](#базовые-плагины-core)
      - [1. `maven-compiler-plugin`](#1-maven-compiler-plugin)
      - [2. `maven-surefire-plugin` vs `maven-failsafe-plugin`](#2-maven-surefire-plugin-vs-maven-failsafe-plugin)
      - [3. `spring-boot-maven-plugin`](#3-spring-boot-maven-plugin)
    - [Плагины Качества (Quality Gates)](#плагины-качества-quality-gates)
      - [4. `maven-enforcer-plugin` (Полиция проекта)](#4-maven-enforcer-plugin-полиция-проекта)
      - [5. `jacoco-maven-plugin` (Code Coverage)](#5-jacoco-maven-plugin-code-coverage)
      - [6. `spotbugs-maven-plugin` / `maven-checkstyle-plugin`](#6-spotbugs-maven-plugin--maven-checkstyle-plugin)
    - [Плагины Поставки (Delivery)](#плагины-поставки-delivery)
      - [7. `versions-maven-plugin`](#7-versions-maven-plugin)
      - [8. `cyclonedx-maven-plugin` (Security)](#8-cyclonedx-maven-plugin-security)
  - [4. Multi-module Projects (Reactor)](#4-multi-module-projects-reactor)
    - [Parent POM](#parent-pom)
    - [Reactor Commands](#reactor-commands)
  - [5. Maven Wrapper (`mvnw`)](#5-maven-wrapper-mvnw)
- [Gradle Deep Dive (Build Engineering)](#gradle-deep-dive-build-engineering)
  - [1. Философия: DSL и Build Lifecycle](#1-философия-dsl-и-build-lifecycle)
    - [Жизненный цикл (Trap for Beginners)](#жизненный-цикл-trap-for-beginners)
    - [Kotlin DSL (`build.gradle.kts`)](#kotlin-dsl-buildgradlekts)
  - [2. Управление зависимостями: Configuration Hell](#2-управление-зависимостями-configuration-hell)
    - [Основные конфигурации:](#основные-конфигурации)
    - [Version Catalogs (`libs.versions.toml`) — Стандарт Enterprise](#version-catalogs-libsversionstoml--стандарт-enterprise)
  - [3. Топ Плагинов для Gradle](#3-топ-плагинов-для-gradle)
    - [Core \& Frameworks](#core--frameworks)
      - [1. `org.springframework.boot` \& `io.spring.dependency-management`](#1-orgspringframeworkboot--iospringdependency-management)
      - [2. `java-library` vs `java`](#2-java-library-vs-java)
    - [Code Quality](#code-quality)
      - [3. `com.diffplug.spotless` (Must Have)](#3-comdiffplugspotless-must-have)
      - [4. `io.gitlab.arturbosch.detekt` (Для Kotlin)](#4-iogitlabarturboschdetekt-для-kotlin)
    - [Shadow \& Packaging](#shadow--packaging)
      - [5. `com.github.johnrengelman.shadow`](#5-comgithubjohnrengelmanshadow)
    - [Security](#security)
      - [6. `org.owasp.dependencycheck`](#6-orgowaspdependencycheck)
  - [4. Gradle Wrapper (`gradlew`) — Это закон](#4-gradle-wrapper-gradlew--это-закон)
  - [5. Performance Tuning (Как ускорить билд)](#5-performance-tuning-как-ускорить-билд)
    - [Remote Build Cache (Для команд)](#remote-build-cache-для-команд)
- [Дополнительно: Server-Side Rendering (SSR) \& WebJars](#дополнительно-server-side-rendering-ssr--webjars)
  - [1. Thymeleaf: Стандарт Spring Boot](#1-thymeleaf-стандарт-spring-boot)
    - [Философия: Natural Templates](#философия-natural-templates)
    - [Use Cases в Банке:](#use-cases-в-банке)
    - [Прод-нюансы:](#прод-нюансы)
  - [2. Apache FreeMarker: Старая гвардия](#2-apache-freemarker-старая-гвардия)
    - [Философия: Generic Template Engine](#философия-generic-template-engine)
    - [Use Cases:](#use-cases)
    - [FreeMarker vs Thymeleaf](#freemarker-vs-thymeleaf)
  - [3. WebJars: Frontend для бэкендеров](#3-webjars-frontend-для-бэкендеров)
    - [Как это работает](#как-это-работает-1)
    - [Locator (Избавляемся от версий)](#locator-избавляемся-от-версий)
    - [Verdict Senior-разработчика:](#verdict-senior-разработчика)
