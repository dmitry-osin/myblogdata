...
**Конец Уровня 6.**
Мы научились "обманывать" базу данных: хранить одно, а показывать другое (Конвертеры), и делать вид, что удалили, хотя просто спрятали (Soft Delete).
...
3. Если перевод прошел, но сохранение истории упало — деньги должны вернуться (атомарность).
4. Написать тест на **Testcontainers**, который запускает 10 параллельных потоков, гоняющих деньги между двумя счетами. В конце сумма на двух счетах должна остаться неизменной (консистентность).
